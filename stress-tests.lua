...
function stressunit(unit)
	if dfhack.units.isCitizen(unit) then
		if stress_target > -1 then
			unit.status.current_soul.personality.stress_level = stress_target
		else
			local temp = unit.status.current_soul.personality.stress_level
			if stress_factor ~= 0 then
				unit.status.current_soul.personality.stress_level = temp + math.floor(stress_factor*stress_step) + math.random(0, math.floor(stress_factor*stress_step))
			else
				unit.status.current_soul.personality.stress_level = temp + stress_step + math.random(0, stress_step)
			end
		end
	end
end
